...
function ErrorBanner({ language, operation, detail, onRetry }: { language: Language; operation: FailedOperation; detail?: string | null; onRetry(): void }) {
  ...
  return <div className="error-panel" role="alert"><span aria-hidden="true">!</span><div><strong>{failure.title}</strong><p>{detail ?? failure.detail}</p></div><button type="button" className="secondary-action" onClick={onRetry}>{failure.retry}</button></div>;
}
...
export function App({ api }: { api: AnalysisApi }) {
  ...
  const runAnalysis = async () => {
    if (!project.plan || !project.planApproved) return;
    const current = ++runId.current;
    setRunning(true); setCancelled(false); setFailedOperation(null); setSafeErrorDetail(null); setJobProgress({ progress: 0, message: null }); project.setActiveStep("run");
    try {
      const results = await api.runAnalysis(project.plan, (progress) => {
        if (runId.current === current) setJobProgress({ progress: progress.progress, message: progress.message });
      });
  ...
  const changePlanApproval = async (next: boolean) => {
    if (!next || !project.plan) {
      project.setPlanApproved(false);
      return;
    }
    try {
      await api.approvePlan(project.plan);
      project.setPlanApproved(true);
    } catch (error) {
      setFailedOperation("plan");
      if (error instanceof AnalysisApiError) setSafeErrorDetail(error.message);
    }
  };
